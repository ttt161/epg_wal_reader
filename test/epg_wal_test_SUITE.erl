...
wal_reader_base_test(_C) ->
    ...
    {ok, C} = epgsql:connect(DbOpts),
    %% INSERT test
    {ok, 1} = epgsql:equery(C, "INSERT INTO t1 (
    ...
    %% UPDATE test
    {ok, 1} = epgsql:squery(C, "UPDATE t1 SET jsonb_array = ARRAY[
        ARRAY[
    ...
    %% DELETE test
    {ok, 1} = epgsql:squery(C, "DELETE FROM t1 where int2 = 32767"),
...
wal_persistent_slot_test(_C) ->
    ...
    {ok, C} = epgsql:connect(DbOpts),
    {ok, _} = epgsql:with_transaction(
        C,
        fun(Conn) ->
            {ok, _} = epgsql:equery(Conn, "INSERT INTO t2 (int2, varchar, bytea, jsonb) VALUES
                (1, 'example', $1, '{\"name\": \"Alice\", \"age\": 30, \"active\": true}')", [<<"BYTES1">>]
            ),
            {ok, _} = epgsql:equery(Conn, "UPDATE t2 SET bytea = $1 WHERE int2 = 1", [<<"BYTES2">>])
        end
    ),
    {ok, [
    ...
    timer:sleep(100),
    {ok, _} = epgsql:equery(C, "UPDATE t2 SET bytea = $1 WHERE int2 = 1", [<<"BYTES3">>]),
    {ok, _} = epgsql:squery(C, "INSERT INTO t2 (int2, varchar) VALUES (2, 'example2')"),
    {ok, _} = epgsql:squery(C, "INSERT INTO t2 (int2, varchar) VALUES (3, 'example3')"),
    {ok, _} = epgsql:squery(C, "INSERT INTO t2 (int2, varchar) VALUES (4, 'example4')"),
